...
namespace nyas
{
    ...
    void test_inersesqrt()
    {
        ...
        ::std::cout << "relative error: " << ((fast_res - default_res) / exam) << ::std::endl;
    ...
    void example_rendering_output()
    {
        ...
        save_bmp((output_dir + "gradient_color.bmp").c_str(), ibuff);
    ...
    void example_samplers()
    {
        ...
        if (makedir(sampler_output_dir.c_str()) != 0) {
        ...
        save_bmp((sampler_output_dir + "regular.bmp").c_str(), buff);

        // pure random sampling
        buff.for_each(clean_to_black);
        samplers::PureRandom samp2(8);
        plot_point(buff, samp2);
        save_bmp((sampler_output_dir + "pureRandom.bmp").c_str(), buff);

        // jittered sampling
        buff.for_each(clean_to_black);
        samplers::Jittered samp3(8);
        plot_point(buff, samp3);
        save_bmp((sampler_output_dir + "jittered.bmp").c_str(), buff);

        // n-Rooks sampling
        buff.for_each(clean_to_black);
        samplers::NRooks samp4(8);
        plot_point(buff, samp4);
        save_bmp((sampler_output_dir + "n-Rooks.bmp").c_str(), buff);

        // multi-jittered sampling
        buff.for_each(clean_to_black);
        samplers::MultiJittered samp5(8);
        plot_point(buff, samp5);
        save_bmp((sampler_output_dir + "multiJittered.bmp").c_str(), buff);

        // hammersley sampling
        buff.for_each(clean_to_black);
        samplers::Hammersley samp6(8);
        plot_point(buff, samp6);
        save_bmp((sampler_output_dir + "hammersley.bmp").c_str(), buff);
    ...
    void example_cameras()
    {
        ...
        if (makedir(camera_output_dir.c_str()) != 0) {
        ...
        */
        ...
        auto render_and_output = [&render_ray] (Camera & camera, char const* output_path) {
            ...
            save_bmp(output_path, camera.figure().map<uint8>(to_display));
        };
        ...
        render_and_output(*camera1, (camera_output_dir + "parallele1.bmp").c_str());
